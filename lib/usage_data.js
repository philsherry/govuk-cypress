...
exports.getUsageDataConfig = function () {
  ...
  let usageDataConfig = {};
  try {
    usageDataConfig = require(path.join(
      __dirname,
...
exports.setUsageDataConfig = function (usageDataConfig) {
  ...
  try {
    fs.writeFileSync(
      path.join(__dirname, '../usage-data-config.json'),
      usageDataConfigJSON
    );
    return true;
  } catch (error) {
...
exports.askForUsageDataPermission = function () {
  return new Promise(function (resolve, reject) {
    const description = fs
      .readFileSync(path.join(__dirname, 'usage-data-prompt.txt'), 'utf8')
      .trim();
...
exports.startTracking = function (usageDataConfig) {
  ...
  const trackingUser = universalAnalytics(
    trackingId,
    usageDataConfig.clientId
  );
